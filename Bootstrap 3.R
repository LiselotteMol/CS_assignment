...
index = 1
f1_star_results_3 = rep(NA, length(r))
pq_results_3 = rep(NA, length(r))
pc_results_3 = rep(NA, length(r))
...
for(i in seq_along(r)) {
  ...
  results_3 = f1_star(candidate_test, real_dupl_test, bootstrap_test$modelID)
  f1_star_results_3[index]= results_3[3]
  pq_results_3[index] = results_3[1]
  pc_results_3[index] = results_3[2]
  #print(f1_star_results_3[index])
  index = index + 1
}


max_h_3 = rep(NA, length(r))
index = 1
for(i in seq_along(r)) {
  candidate_train = candidates(loop, signature_train)
  ...
  max_h_3[index] <- h_seq[best_h[1]]
  index = index + 1
}


possible_comparisons_3 = rep(NA, length(r))
foc_3 = rep(NA, length(r))
f1_test_3 = rep(NA, length(r))
...
for(i in seq_along(r)) {
  ...
  cut_test <- cutree(hc_test, h = max_h_3[index])
  ...
  possible_comparisons_3 = (nrow(bootstrap_test) *(nrow(bootstrap_test)-1)) / 2
  foc_3[index] = length(which(candidate_test==1)) / possible_comparisons_3
  
  f1_test_3[index] = f1(clusters_test, real_dupl_test)
  index = index + 1
}
