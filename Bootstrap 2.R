...
index = 1
f1_star_results_2 = rep(NA, length(r))
pq_results_2 = rep(NA, length(r))
pc_results_2 = rep(NA, length(r))
...
for(i in seq_along(r)) {
  ...
  results_2 = f1_star(candidate_test, real_dupl_test, bootstrap_test$modelID)
  f1_star_results_2[index]= results_2[3]
  pq_results_2[index] = results_2[1]
  pc_results_2[index] = results_2[2]
  index = index + 1
}
f1_star_results_2 <- as.matrix(f1_star_results_2)


max_h_2 = rep(NA, length(r))
index = 1
for(i in seq_along(r)) {
  ...
  max_h_2[index] <- h_seq[best_h[1]]
  index = index + 1
}

possible_comparisons_2 = rep(NA, length(r))
foc_2 = rep(NA, length(r))
f1_test_2 = rep(NA, length(r))
...
for(i in seq_along(r)) {
  ...
  cut_test <- cutree(hc_test, h = max_h_2[index])
  ...
  possible_comparisons_2 = (nrow(bootstrap_test) *(nrow(bootstrap_test)-1)) / 2
  foc_2[index] = length(which(candidate_test==1)) / possible_comparisons_2
  
  f1_test_2[index] = f1(clusters_test, real_dupl_test)
  index = index + 1
}
