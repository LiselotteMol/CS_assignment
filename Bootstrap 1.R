...
index = 1
f1_star_results = rep(NA, length(r))
pq_results = rep(NA, length(r))
pc_results = rep(NA, length(r))


for(i in seq_along(r)) {
  candidate_test = candidates(i, signature_test)
  results = f1_star(candidate_test, real_dupl_test, bootstrap_test$modelID)
  f1_star_results[index]= results[3]
  pq_results[index] = results[1]
  pc_results[index] = results[2]
  index = index + 1
}


max_h = rep(NA, length(r))
index = 1
for(i in seq_along(r)) {
  ...
  max_h[index] <- h_seq[best_h[1]]
  index = index + 1
}

possible_comparisons = rep(NA, length(r))
foc = rep(NA, length(r))
f1_test = rep(NA, length(r))
...
for(i in seq_along(r)) {
  ...
  cut_test <- cutree(hc_test, h = max_h[index])
  ...
  possible_comparisons = (nrow(bootstrap_test) *(nrow(bootstrap_test)-1)) / 2
  foc[index] = length(which(candidate_test==1)) / possible_comparisons
  
  f1_test[index] = f1(clusters_test, real_dupl_test)
  index = index + 1
}
